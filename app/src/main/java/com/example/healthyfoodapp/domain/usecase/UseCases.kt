...
class AddDishUseCase(private val repository: DishRepository) {
    operator fun invoke(name: String, type: String, calories: String, description: String): Long {
        ...
        return repository.addDish(
            Dish(name = name, type = type, calories = calories,
...
class SaveMealToCatalogUseCase(private val repository: DishRepository) {
    operator fun invoke(detail: MealDetail): Long {
        ...
        return repository.addDish(
            Dish(name = detail.name, type = detail.category,
                calories = "0", description = description, categoryId = categoryId)
        )
    }
}